...
public class ImageDownloader {
    ...
    public ImageDownloader(Context context) {
        ...
        cacheFileDir = SDUtils.createFileDir(context, DIR_CACHE);
    }
    ...
    public void loadImage(final String url, final int width, final int height,
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                long cacheFileSize = SDUtils.getFileSize(cacheFileDir);
                if (cacheFileSize > DIR_CACHE_LIMIT) {
   
                    SDUtils.delFile(cacheFileDir, false);
                    taskCollection.clear();
                }
                // 缓存文件名称（ 替换url中非字母和非数字的字符，防止系统误认为文件路径）
                String urlKey = url.replaceAll("[^\\w]", "");
                // 将Bitmap加入文件缓存
                SDUtils.savaBitmap(cacheFileDir, urlKey, bitmap);
            }
        };
    ...
    public Bitmap getBitmapCache(String url) {
        ...
        } else if (SDUtils.isFileExists(cacheFileDir, urlKey)
                && SDUtils.getFileSize(new File(cacheFileDir, urlKey)) > 0) {
            // 从文件缓存中获取Bitmap
